!function (e, t) {
  ...
  return function (e, i) {
    var a, o, l = Number.MAX_VALUE, c = 1;
    if (null === e) return !1;
    (a = e).init = function (e) {
      ...
      o = !matchMedia("(hover: none)").matches, u(), window.on("resize", u), a.options.overlay && (a.overlayPanel = document.createElement("div"), a.overlayPanel.classList.add("overlay-panel"), a.overlayPanel.style.background = a.options.overlayColor, a.appendChild(a.overlayPanel), a.overlayPanel.addEventListener("click", a.toggleOffcanvas)),
    ...
    var f = function (e) {
        for (var t = 0; t < a.menuLinks.length; t++) a.menuLinks[t].on(e, function (e) {
          if (e.target.parentNode.classList.contains("has-submenu")) {
            ...
            if (e.target.parentNode.classList.remove("is-active"), e.target.getElementsByClassName("submenu-indicator").length > 0 && e.target.lastElementChild.classList.remove("is-active"), a.hideSubmenus(e.target), "_blank" === e.target.getAttribute("target") || "blank" === e.target.getAttribute("target")) window.open(e.target.getAttribute("href")); else {
        ...
        for (var e = 0; e < a.menuItems.length; e++) a.menuItems[e].classList.contains("has-submenu") && (a.menuItems[e].on("mouseenter.item", function (e) {
          ...
          if (e.preventDefault(), e.stopPropagation(), e.target.classList.contains("has-submenu")) {
            var t = (n = e.target.lastElementChild, {x: (i = n.getBoundingClientRect()).left, y: i.top});
      ...
      b = function () {
        var e = a.querySelectorAll(".navigation-link[href*='#']"), t = function (e) {
          for (var t = [], n = 0; n < e.length; n++) {
            var i = e[n].getAttribute("href");
            if (i.length > 1 && "#" === i.substring(0, 1)) {
              var a = document.getElementById(i.substr(1)), o = Math.floor(a.offsetTop),
          ...
          var o = {
            ...
            }, s = window.pageYOffset, r = "now" in window.performance ? performance.now() : (new Date).getTime(),
            ...
            u = "number" == typeof e ? e : e.offsetTop + a.options.scrollSpyOffset, m = Math.round(l - u < c ? l - c : u);
          if ("requestAnimationFrame" in window == 0) return window.scroll(0, m), void (i && i());
          !function e() {
            var a = "now" in window.performance ? performance.now() : (new Date).getTime(), l = Math.min(1, (a - r) / t),
              c = o[n](l);
